class full_parity_even_test extends uart_base_test;
    `uvm_component_utils(full_parity_even_test)

    uart_sequence lhs_seq;
    uart_sequence rhs_seq;

    function new(string name = "full_parity_even_test",uvm_component parent);
        super.new(name,parent);
    endfunction

    virtual function void build_phase(uvm_phase phase);
        super.build_phase(phase);
        if(!lhs_cfg.randomize() with {parity==uart_configuration::EVEN;direction == uart_configuration::DUAL;})
